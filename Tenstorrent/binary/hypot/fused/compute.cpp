...
void MAIN {
  ...
  for (uint32_t t = 0; t < per_core_tile_cnt; ++t) {
    ...
    binary_op_init_common(cb_2, cb_3,
                          cb_out); // out CB is irrelevant until we pack
    ...
    unary_op_init_common(cb_out,
                         cb_out); // neutral binding for packer to cb_out
    sqrt_tile_init<false>();
    sqrt_tile<false>(dst_reg);
