...
void MAIN {
    ...
    for (uint32_t t = 0; t < per_core_tile_cnt; ++t) {
        // Micro-tune: do both square(a) and square(b) inside one acquire_dst/release_dst pair
        acquire_dst();
        
        // For A: cb_wait_front(in0,1) → copy_tile(in0,0,0) → square_tile(0) → cb_reserve_back(c_2,1) → pack_tile(0,c_2) → cb_pop_front(in0,1) → cb_push_back(c_2,1)
        ...
        // For B: same into c_3 (reusing the same dst register)
